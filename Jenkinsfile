pipeline {
    ...
    stages {
        ...
        stage('Smoke Test Prod') {
            steps {
                sh '''
                  kubectl rollout status deployment/demo-app-prod -n prod --timeout=90s
                  kubectl run smoke-prod --rm -i --restart=Never -n prod \
                    --image=curlimages/curl -- curl -sf http://demo-app-prod:8080/health
                '''
            }
        }
    }
